...
// Initial string used to obtain the path of this file (webchartsLoader.js)
var callerSite = "";

// A function to calculate the absolute path to the above js files because currently, it uses the baseURI of the executing html file
function getPathRelativeToBase(basePath, relPath) {
    return [basePath, relPath].join('/');
}
...
function loadJS() {
    ...
    if (callerSite.length == 0) {
        callerSite = document.currentScript.src;
    }

    jsElm.src = getPathRelativeToBase(callerSite, arr.shift());
    jsElm.defer = true;
    jsElm.onload = loadJS;
